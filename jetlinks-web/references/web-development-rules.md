...
6. 编码前执行组件抽离与复用门禁：确认要改哪些 Vue/JSX/TSX 文件、每个文件职责边界、是否存在职责混杂或可独立抽出的功能块、是否已有组件 / hooks / utils / services 可复用；约 300 行只作为额外复查信号。
7. 若重复结构出现 2 次以上、请求或业务编排准备进入组件，或同一文件混入页面壳层 / 可复用 UI / 状态编排 / 数据转换等多个职责，先按功能拆分到子组件、hook、service/api 或 util，再实现；不要只为了压缩行数制造空壳组件。
...
- Vue/JSX/TSX 文件不以 300 行作为硬性拆分线；接近或超过 300 行时必须复查职责边界，只有存在职责混杂、重复结构、请求 / 业务编排、数据转换污染 UI 组件等问题时才拆分。
- 新增组件、hook、service 或 util 前必须先查已有能力；能复用或配置扩展时，不新增平行实现。
- 页面组件只保留布局组合、状态展示和事件分发；请求、参数组装、业务编排、复杂计算不应塞进 UI 组件。不要把“超过 300 行”简单处理成空壳组件拆分；应先按功能边界抽出子组件、hook、service/api 或 util，并在最终回复中说明职责复查和必要的行数情况。
- 业务逻辑默认保持连贯，反对碎片化。除非某段逻辑具有高通用复用价值、单个方法确实过长（例如约 80 行以上）或能隔离明确副作用 / 数据转换 / 公共契约，否则不要把完整业务流程拆成大量只调用一次的微型私有方法。
- 可读性优先于过度设计；如果当前方法能让业务流程一眼看到底，优先保留在原处，避免为了“抽象”让读者在多个方法间频繁跳转。
- 设计文档里的业务规则、状态流转和异常分支必须映射到具体实现附近的业务注释；注释说明触发条件、业务目的和影响结果，禁止“需求要求 / 根据文档 / 这里校验 / 处理数据”等空泛机械表述，修改逻辑时同步更新对应注释。
...
- 复杂业务规则、状态流转和异常分支是否已有贴近实现代码的业务注释，且注释没有堆在类头 / 方法头、没有使用空泛机械表述、没有与代码行为不一致。
