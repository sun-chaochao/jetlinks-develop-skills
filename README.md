...
用于共享编码规范、注解/导入确认、命名约束、i18n 判断与实现，以及 TraceHolder 链路追踪判断。
...
- 只想确认代码规范、导入、i18n 判断、国际化实现或 TraceHolder 埋点边界：`$jetlinks-conventions`
...
- 目的：为什么要做这次改动
- 核心变动：改了哪些模块、行为和边界
- 设计与测试目标：较大后端改动或新功能需列出设计稿路径、确认状态、测试目标达成情况、CRUD / AssetsHolder 数据权限分析结论和链路追踪结论
